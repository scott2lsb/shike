...
public class Activity_SelectSubjectOrTeacher extends Activity implements
		OnClickListener {
	static int ISSUBJECT_CODE = 101;
	static int ISTEACHER_CODE = 102;
	private ListView mListview;
	private SubjectAdapter subjectAdapter;
	private SKTeacherOrSubject item;
	static String ISTEACHER = "isTeacher";
	static String ISSUBJECT = "isSubject";
	...
	private void skGetSubject() {
		SKAsyncApiController.skGetSubject(new MyAsyncHttpResponseHandler(this,
				true) {
			@Override
			public void onSuccess(String arg0) {
				super.onSuccess(arg0);
				ArrayList<SKTeacherOrSubject> subjects = SKResolveJsonUtil.getInstance().resolveSubject(arg0);
				subjectAdapter = new SubjectAdapter(subjects,Activity_SelectSubjectOrTeacher.this);
	...
	private void skGetTeacher(final String subjectId) {
		SKAsyncApiController.skGetMyTeacher(new MyAsyncHttpResponseHandler(
			...
			public void onSuccess(String arg0) {
				...
				mListview.setAdapter(subjectAdapter);
				if (subjects.size() == 0) {
					AlertDialog.Builder dialog = new Builder(Activity_SelectSubjectOrTeacher.this);
					dialog.setTitle("提示");
