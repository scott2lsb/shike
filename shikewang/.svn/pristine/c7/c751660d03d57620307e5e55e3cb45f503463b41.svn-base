...
//		this.context = context;
	...
	public boolean onTouchEvent(MotionEvent event, int h) {
		int antion = event.getAction();
		if (antion == MotionEvent.ACTION_CANCEL) {
			return false;
		}
		float touchX = event.getX();
		float touchY = event.getY() - h - getTop();
...
//		canvas.drawBitmap(newbit,new Matrix(), mPaint);
	...
	public String getBigBitmapUrl() {
        ...
        drawMainPallent(canvas);
		ImageUtil.writeBitmap(resultBitmap, bgBitmap_path);//此处保存图片至本地大约耗时2S.
		return bgBitmap_path;
	}
    // 获取处理过的图片
